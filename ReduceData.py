...
class ReduceData:
        ...
        def __add_data__(self, data : tuple):
                ...
                con_BERTScore   = lambda acc, elem : {\
                        'P' : acc['P'] + elem[0]['P'].tolist(),\
                        'R' : acc['R'] + elem[0]['R'].tolist(),\
                        'F1' : acc['F1'] + elem[0]['F1'].tolist()}
                ...
                con_BERTScore_f  = lambda acc, elem : {\
                        'P' : np.concatenate(
                                (acc['P'], np.asarray(elem[0]['P'])[np.asarray(elem[3]) if len(elem[3]) != 0 else np.arange(len(elem[0]))] )
                                ),\
                        'R' : np.concatenate(
                                (acc['R'], np.asarray(elem[0]['R'])[np.asarray(elem[3]) if len(elem[3]) != 0 else np.arange(len(elem[0]))] )
                                ),\
                        'F1' : np.concatenate(
                                (acc['F1'], np.asarray(elem[0]['F1'])[np.asarray(elem[3]) if len(elem[3]) != 0 else np.arange(len(elem[0]))])
                                )
                        }
                ...
                con_BLEURT_f = lambda acc, elem : {\
                        'BLEURT': np.concatenate((acc['BLEURT'], np.asarray(elem[1]['BLEURT'])[np.asarray(elem[3]) if len(elem[3]) != 0 else np.arange(len(elem[1]))]))
                        }

                # TODO correct Peterson to Peterson
                con_ME = lambda acc, elem : {\
                        'Peterson': acc['Peterson'] + [elem[2]['Peterson']],\
                ...
                for i, degree in enumerate(self.steps):
                        ...
                        # accumulate data
                        combined_data['BERTScore']['data'][i] = (degree, functools.reduce(con_BERTScore, data[i][1], {'P': [], 'R' : [], 'F1' : []}))
                        combined_data['BERTScore_f']['data'][i] = (degree, functools.reduce(con_BERTScore_f, data[i][1], {'P': [], 'R' : [], 'F1' : []}))
                        combined_data['BLEURT']['data'][i] = (degree, functools.reduce(con_BLEURT, data[i][1], {'BLEURT' : []}))
                        combined_data['BLEURT_f']['data'][i] = (degree, functools.reduce(con_BLEURT_f, data[i][1], {'BLEURT' : []}))
                        combined_data['ME']['data'][i] = (degree, functools.reduce(con_ME, data[i][1], {'Peterson' : [], 'Schnabel' : [], 'CAPTURE' : []}))
                        ...
                        # # del 'data'
                        # del combined_data['BERTScore']['data'][i]
                        # del combined_data['BERTScore_f']['data'][i]
                        # del combined_data['BLEURT']['data'][i]
                        # del combined_data['BLEURT_f']['data'][i]
                        # del combined_data['ME']['data'][i]
